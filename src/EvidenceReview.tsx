...
import { criterionEstimate, evidenceStages, guidanceLevels } from './evidence'
...
function loadChecks(key:string): Record<string, Check> {
...
export function EvidenceReview({ name, role, team, progress, task, title, criteria, review, onReview, onSave, onTeam }: {
  ...
  const {scenario:s,data:project}=useWorkspace()
  const data = scenarioEvidence(s,name,role,team,task==='vlan',project)
  const [active, setActive] = useState('E02')
  const [stage, setStage] = useState<number | null>(null)
  const [supportIndex, setSupportIndex] = useState(1)
  const [checks, setChecks] = useState(()=>loadChecks(workspaceKey(s.id,'trace-evidence-checks')))
  ...
  function saveCheck() {
    try { localStorage.setItem(workspaceKey(s.id,'trace-evidence-checks'), JSON.stringify(checks)); setCheckNotice(`${evidence.id} 교수자 확인을 저장했습니다.`) }
  ...
  return <>
    ...
    <div className="insight-pair">
      <section className="insight strength"><div className="insight-heading"><span className="insight-symbol" aria-hidden="true"><span className="icon icon-check" /></span><h3>근거에서 보이는 강점</h3></div><strong>{s.strength[0]}</strong><p>{s.strength[1]}</p><button className="text-button" onClick={() => openEvidence('E02')}>E02 · 비교 근거 보기 <span aria-hidden="true">↗</span></button></section>
      <section className="insight gap"><div className="insight-heading"><span className="insight-symbol" aria-hidden="true">?</span><h3>다음에 확인할 부분</h3></div><strong>{!data.hasModification ? '어떤 근거로 수정안을 고를 것인가?' : data.incomplete ? '수정 이후 목표를 충족했는가?' : '다른 조건에서도 같은 판단을 할 수 있는가?'}</strong><p>{!data.hasModification ? s.gap[0] : data.incomplete ? s.gap[1] : s.gap[2]}</p><button className="text-button" onClick={() => openEvidence(data.incomplete ? 'E05' : 'E04')}>{!data.hasModification ? 'E05 · 진단 계획 보기' : data.incomplete ? 'E05 · 검증 계획 보기' : 'E04 · 검증 범위 보기'} <span aria-hidden="true">↗</span></button></section>
    </div>
    <section className="panel competency-panel"><div className="section-heading"><h2>학습 상태와 수행 근거</h2><span className="count-label">AI 정리 · 확인 전</span></div>
      ...
      <div className="reference-tables"><section><h3>교과 개념별 확인 포인트</h3><table className="reference-table"><thead><tr><th>교과 개념</th><th>확인 상태</th><th>근거</th></tr></thead><tbody>{[[s.concepts[0],'E01','관찰로 범위 구분'],[s.concepts[1],'E02','비교 근거 있음'],['검증 조건과 결과 해석',data.incomplete?'E05':'E04',data.incomplete?'실행 결과 미수집':'검증 범위 추가 확인']].map(([concept,id,state],i)=><tr key={concept}><th scope="row">{concept}</th><td className={i===2?'reference-caution':'reference-positive'}>{state}</td><td><button className="text-button" onClick={()=>openEvidence(id)}>{id}<span className="icon icon-right" aria-hidden="true" /></button></td></tr>)}</tbody></table></section>
    ...
    <section className="panel"><div className="section-heading"><h2>지식 연결과 적용 계획</h2><span className="count-label">팀 수행 맥락</span></div><KnowledgeConnections team={team} /></section>
    ...
    <section id="guidance-history" className="panel guidance-panel"><div className="section-heading"><h2>L1–L4 가이던스 흐름</h2><span className="count-label">지원 수준 · 능력 점수 아님</span></div><p className="section-note">막힐 때는 지원을 넓히고, 스스로 진행하면 확인 질문으로 줄입니다.</p>
      <div className="guidance-visual"><div className="guidance-chart"><h3>지원 수준의 변화 <span>총 {data.guidance.length}회</span></h3><svg viewBox="0 0 620 248" role="img" aria-label={data.guidance.map(g=>`${g.time} L${g.level} ${guidanceLevels[g.level-1]}`).join(', ')}>{[4,3,2,1].map((level,i)=><g key={level}><rect className={`chart-band-${level}`} x="42" y={10+i*50} width="565" height="44" rx="7" /><text x="4" y={37+i*50}>L{level}</text><line x1="42" x2="607" y1={32+i*50} y2={32+i*50} /></g>)}{data.guidance.slice(1).map((g,i)=><path key={i} className={`chart-stroke-${data.guidance[i].level}`} d={`M ${64+i*520/(Math.max(1,data.guidance.length-1))} ${182-(data.guidance[i].level-1)*50} H ${64+(i+1)*520/(Math.max(1,data.guidance.length-1))} V ${182-(g.level-1)*50}`} />)}{data.guidance.map((g,i)=><g key={i}><circle className={`chart-dot-${g.level} ${i===supportIndex?'active':''}`} cx={64+i*520/(Math.max(1,data.guidance.length-1))} cy={182-(g.level-1)*50} r={i===supportIndex?9:6} /><text x={64+i*520/(Math.max(1,data.guidance.length-1))} y="231" textAnchor="middle">{g.time}</text></g>)}</svg></div>
