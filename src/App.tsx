  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main id="main" className="main">
      ...
      {step === 1 ? <div className="design-grid">
        ...
        <Section number="03" title="과제 목표" className="goals-panel" action={<button type="button" className="text-button" onClick={openSuggestions}><Icon name="download" />과목 설정에서 불러오기</button>}>
        ...
        <Section number="06" title="평가 기준" className="criteria-panel" action={<button type="button" className="text-button" onClick={() => { update('criteria', courseCriteria.map(c => ({...c}))); setMessage('과목 평가 기준을 불러왔습니다. 과제에 맞게 수정할 수 있어요.') }}><Icon name="download" />과목 기준 불러오기</button>}>
