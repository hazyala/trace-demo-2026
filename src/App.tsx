...
import { type Assignment as AssignmentType, courseCriteria, modes, readDraft, storageKey, suggestGoals, validateAssignment } from './assignment'
...
function App() {
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main id="main" className="main">
      ...
      <div className="workflow"><div className="step-tabs" aria-label="과제 생성 단계">{['과제 설계', 'AI 운영 방식', '학생 미리보기'].map((label,i) => <div className="step-item" key={label}>{i > 0 && <span className="step-connector" />}<button className={step === i+1 ? 'current' : step > i+1 ? 'complete' : ''} aria-current={step === i+1 ? 'step' : undefined} onClick={() => changeStep(i+1)}><span>{step > i+1 ? <Icon name="check" /> : i+1}</span>{label}</button></div>)}</div><span className="step-count">STEP {String(step).padStart(2, '0')} <span>/ 03</span></span></div>
